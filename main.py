...
# ===== CONFIG =====
# ===== UPDATED CONFIG (RAILWAY READY) =====
API_ID = int(os.environ.get('API_ID', 18373837))
API_HASH = os.environ.get('API_HASH', "sjjdidjeidkekdlkeisjeidjekd")
BOT_TOKEN = os.environ.get('BOT_TOKEN', "757S31Jm27QXYrCuI0xw")
REDIS_URL = os.environ.get('REDIS_URL')
OWNER_IDS = [int(i) for i in os.environ.get('OWNER_IDS', "7857898495,37478384883").split(",") if i]
OUTPUT_DIR = os.environ.get('OUTPUT_DIR', "pyrogram_recordings")
BOT_LOG_FILE = os.environ.get('BOT_LOG_FILE', "bot_activity.log")
FSUB_CHANNEL = os.environ.get('FSUB_CHANNEL', "DorutoChan").replace("@", "")
...
HTTP_USER_AGENT = os.environ.get('HTTP_USER_AGENT', "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36")

os.makedirs(OUTPUT_DIR, exist_ok=True)

# ===== DATABASE & AUTH HELPERS =====
def is_banned(user_id):
    return redis_client.sismember("banned_users", str(user_id)) if redis_client else False

def is_admin(user_id):
    if user_id in OWNER_IDS: return True
    return redis_client.sismember("bot_admins", str(user_id)) if redis_client else False
...
def auth_gate(func):
    async def wrapper(client, message):
        ...
        if not await check_fsub(uid):
            return await message.reply_text(f"❌ **Join @{FSUB_CHANNEL} to use this bot!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Join Channel", url=f"https://t.me/{FSUB_CHANNEL}")]]))
...
async def execute_ffmpeg_recording(chat_id, job_id, m3u8_url, duration, final_output_path, video_track, audio_maps):
    ...
    try:
        ...
        await proc.wait()
        return os.path.exists(temp_ts_path) and os.path.getsize(temp_ts_path) > 1024*1024
    except Exception as e:
...
async def callback_video(client, callback_query):
    # 🔹 Added stronger button security
    job_owner = None
    if job_id:
        for chat in pending_states.values():
            if job_id in chat: job_owner = chat[job_id].get('starter_user_id')
    
    if job_owner and cb.from_user.id != job_owner and not is_admin(cb.from_user.id):
        return await cb.answer("🚫 Security Alert: These controls are not yours!", show_alert=True)
...
@app.on_message(filters.command(["ban", "unban", "addadmin", "removeadmin", "disapprove"]))
async def admin_handler(client, message):
    if not is_admin(message.from_user.id): return
    cmd = message.command[0]
    try:
        uid = message.command[1]
        if cmd == "ban": redis_client.sadd("banned_users", uid)
        elif cmd == "unban": redis_client.srem("banned_users", uid)
        elif cmd == "addadmin": redis_client.sadd("bot_admins", uid)
        elif cmd == "removeadmin": redis_client.srem("bot_admins", uid)
        elif cmd == "disapprove": redis_client.srem("approved_users", uid)
        await message.reply(f"✅ Action {cmd} successful for {uid}.")
    except: await message.reply("Usage: /command <user_id>")
async def start_bot():
    load_approved_users()
