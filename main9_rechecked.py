...
FSUB_CHANNEL = os.environ.get("FSUB_CHANNEL", "@DorutoChan")
BRANDING = os.environ.get("BRANDING", "🔗 Join Channel:- @DorutoChan")

OWNER_IDS = [
    int(x.strip())
    for x in os.environ.get("OWNER_IDS", "6407533831").split(",")
    if x.strip().isdigit()
]
...
async def upload_single_file(client, chat_id: int, file_path: str, caption: str, thumb_path: Optional[str] = None,
    ...
    try:
        result = await client.send_video(
            ...
            progress=lambda current, total: asyncio.run_coroutine_threadsafe(update_progress(current, total), BOT_LOOP),
        )
...
async def execute_ffmpeg_recording(job: dict) -> Tuple[bool, str]:
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        ...
        if selected_video:
            cmd.extend(["-map", f"0:{selected_video['index']}", "-c:v", "copy"])
        else:
            cmd.append("-vn")
        ...
        actual_duration = get_duration(temp_output) if os.path.exists(temp_output) else 0
        ...
        if proc.returncode == 0 and file_size > 500 * 1024 and actual_duration >= target_min_duration:
            if finalize_recording(temp_output, final_output_path):
...
async def worker(worker_id: int):
    while True:
        ...
        try:
            ...
            repair_media_file(final_output_path)
...
async def start_handler(client, message):
    text = (
        ...
        "**Admin Commands**\n"
        "`/approve <user_id>` | `/disapprove <user_id>`\n"
